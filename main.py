...
def all_members():
    ...
    details = Label(second_frame, text=tabulate(myresult, headers=['Membership ID', 'First Name', 'Last Name', 'Phone Number', 'Email ID','Points'], tablefmt='fancy_grid'), bg="#101820", fg="#F2AA4C", font="Consolas 12 ").grid(pady=10, padx=20, row=5, column=1)
...
def search_by_ID():
    def search_id():
        ...
        myresult = cur.fetchall()
        #Showing Result
        result = Label(search_result, text=tabulate(myresult, headers=['Membership ID', 'First Name', 'Last Name', 'Phone Number', 'Email ID','Points'], tablefmt='fancy_grid'), bg="#101820", fg="#F2AA4C", font="Consolas 12 ").grid(padx=20,row=5)
...
def display_all_rooms():
       ...
       all_rooms_window.geometry("935x320")
       all_rooms_window.resizable(0, 0)
       all_rooms_window.configure(bg="#2C5F2D")
       cur.execute("SELECT *FROM room")
       myresult=cur.fetchall()
       main_frame = Frame(all_rooms_window, bg="#2C5F2D")
       main_frame.pack(fill=BOTH, expand=1)
       my_canvas = Canvas(main_frame, bg="#2C5F2D")
       my_canvas.pack(side=LEFT, fill=BOTH, expand=1)
       my_scrollbar = ttk.Scrollbar(main_frame, orient=VERTICAL, command=my_canvas.yview)
       my_scrollbar.pack(side=RIGHT, fill=Y)
       my_canvas.configure(bg="#2C5F2D", yscrollcommand=my_scrollbar.set)
       my_canvas.bind('<Configure>', lambda e: my_canvas.configure(scrollregion=my_canvas.bbox("all")))
       second_frame = Frame(my_canvas, bg="#2C5F2D")
       my_canvas.create_window((0, 0), window=second_frame, anchor="nw")
       details=Label(second_frame,text=tabulate(myresult,headers=['Membership ID','First name','Last name','Booking amount','Room_no','Occupancy'],tablefmt='fancy_grid'),bg="#2C5F2D", fg="#F2AA4C", font="Consolas 12 ").grid(pady=10, padx=20, row=0,column=1)
       Label(second_frame, text="", bg="#2C5F2D").grid(row=1)
       Button(second_frame, text="Close Window", font="Corbel 10", width=20, height=2,bd=5, command=all_rooms_window.destroy).grid(row=2, column=1, sticky="s")
...
def clear_cmd_rooms():
       ...
       search_by_rooms_window.configure(bg="#2C5F2D")
       Label(search_by_rooms_window, text="   ╔═══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C").grid(row=1)
       Label(search_by_rooms_window, text="Search By Room no.",bg="#2C5F2D", fg="#F2AA4C", font="Corbel 20 bold").grid(row=2)
       Label(search_by_rooms_window, text="   ╚═══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
       Label(search_by_rooms_window, text="", bg="#2C5F2D").grid(row=4)
       Label(search_by_rooms_window, text="   Enter Room number : ", bg="#2C5F2D",font="Corbel 14 bold", fg="#F2AA4C").grid(row=5, sticky="w")
       def search_rooms():
              search_result=Toplevel()
              search_result.title("Search Result")
              search_result.geometry("935x320")
              search_result.resizable(0, 0)
              search_result.configure(bg="#2C5F2D")
              Label(search_result, text="   ╔═══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C").grid(row=1)
              Label(search_result, text="        Result",bg="#2C5F2D", fg="#F2AA4C", font="Corbel 20 bold").grid(row=2)
              Label(search_result, text="   ╚═══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
              Label(search_result, text="", bg="#2C5F2D").grid(row=4)
              #Data
              
              #rooms_var.set("")
              
              searched=rooms_var.get()
              sql="select * from room WHERE Room_no ='%s';"%searched
              #name=(searched,)
              cur.execute(sql)
              myresult = cur.fetchall()
              #Showing Result
              result = Label(search_result, text=tabulate(myresult, headers=['Membership ID','First name','Last name','Booking amount','Room no'],tablefmt='fancy_grid'), bg="#2C5F2D", fg="#F2AA4C", font="Consolas 12 ").grid(padx=20,row=5)
              Label(search_result, text="", bg="#2C5F2D").grid(row=6)
              Button(search_result,text="Close Window",font="Corbel 10",width=20,height=2,bd=5,command=search_result.destroy).grid(row=7)
                   

       ID_entry = (Entry(search_by_rooms_window, width=15, font="Consolas 14",textvariable=rooms_var).grid(row=5, sticky="e"))
       def clear_input():
           rooms_var.set("")
       Label(search_by_rooms_window, text=" ", bg="#2C5F2D", font="Corbel 20 bold").grid(row=6)
       Label(search_by_rooms_window, text=" ", bg="#2C5F2D", font="Corbel 20 bold").grid(row=7)
       ...
       Label(search_by_rooms_window, text=" ", bg="#2C5F2D").grid(row=8)
...
def clear_cmd_vacancy():
       ...
       search_by_vac_window.configure(bg="#2C5F2D")
       Label(search_by_vac_window, text="   ╔═══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C").grid(row=1)
       Label(search_by_vac_window, text=" Search By Vacancy",bg="#2C5F2D", fg="#F2AA4C", font="Corbel 20 bold").grid(row=2)
       Label(search_by_vac_window, text="   ╚═══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
       Label(search_by_vac_window, text="", bg="#2C5F2D").grid(row=4)
       Label(search_by_vac_window, text="   Enter Occupancy :", bg="#2C5F2D",font="Corbel 14 bold", fg="#F2AA4C").grid(row=5, sticky="w")
       def search_vacancy():
              search_result=Toplevel()
              search_result.title("Search Result")
              search_result.geometry("935x320")
              ...
              sql="select Room_no,Occupancy from room WHERE Occupancy ='%s';"%searched
              #name=(searched,)
              cur.execute(sql)
              myresult = cur.fetchall()
              #Showing Result
              result = Label(search_result, text=tabulate(myresult, headers=['Room_no','Occupancy'],tablefmt='fancy_grid'), bg="#2C5F2D", fg="#F2AA4C", font="Consolas 12 ").grid(padx=20,row=5)
              Label(search_result, text="", bg="#2C5F2D").grid(row=6)
              Button(search_result,text="Close Window",font="Corbel 10",width=20,height=2,bd=5,command=search_result.destroy).grid(row=7)

       ID_entry = (Entry(search_by_vac_window, width=15, font="Consolas 14",textvariable=vac_var).grid(row=5, sticky="e"))
       def clear_input():
           vac_var.set("")
       Label(search_by_vac_window, text=" ", bg="#2C5F2D", font="Corbel 20 bold").grid(row=6)
       Label(search_by_vac_window, text=" ", bg="#2C5F2D", font="Corbel 20 bold").grid(row=7)
       ...
       Label(search_by_vac_window, text=" ", bg="#2C5F2D").grid(row=8)
...
def clear_cmd_update_vacancy():
       ...
       update_vac_window.configure(bg="#2C5F2D")
       Label(update_vac_window, text="   ╔═══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C").grid(row=1)
       Label(update_vac_window, text=" Create Vacancy",bg="#2C5F2D", fg="#F2AA4C", font="Corbel 20 bold").grid(row=2)
       Label(update_vac_window, text="   ╚═══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
       Label(update_vac_window, text="", bg="#2C5F2D").grid(row=4)
       #Label(search_by_vac_window, text="	Enter Vacancy Status. :", bg="#101820",font="Corbel 14 bold", fg="#F2AA4C").grid(row=5, sticky="w")
       def update_vacancy():
              update_result=Toplevel()
              update_result.title("Confirmation")
              update_result.geometry("350x200")
              update_result.resizable(0, 0)
              update_result.configure(bg="#2C5F2D")
              Label(update_result, text="   ╔═════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C").grid(row=1)
              Label(update_result, text="  Updated!",bg="#2C5F2D", fg="#F2AA4C", font="Corbel 20 bold").grid(row=2)
              Label(update_result, text="   ╚═════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
              Label(update_result, text="", bg="#2C5F2D").grid(row=4)
              #SQL Execution
              vac = vac_var.get()
              cur.execute("update room set Occupancy='V' where Room_no ='%s'" %vac)
              myresult = cur.fetchall()    
              db.commit()
              cur.execute("update room set Membership_ID=NULL where Occupancy='V'")
              db.commit()
              cur.execute("update room set First_Name=NULL where Occupancy='V'")
              db.commit()
              cur.execute("update room set Last_Name=NULL where Occupancy='V'")
              db.commit()
              cur.execute("update room set Booking_amount=NULL where Occupancy='V'")
              db.commit()

     
       Label(update_vac_window, text="Enter blah : ", bg="#2C5F2D",fg="#F2AA4C", font="Corbel 15 bold").grid(row=5,sticky='w',padx=20)       
       ID_entry = (Entry(update_vac_window, width=15, font="Consolas 14",textvariable=vac_var).grid(row=5, sticky="e",padx=10))
       def clear_input():
           vac_var.set("")
       Label(update_vac_window, text=" ", bg="#2C5F2D", font="Corbel 20 bold").grid(row=6)
       Label(update_vac_window, text=" ", bg="#2C5F2D", font="Corbel 20 bold").grid(row=7)
       Button(update_vac_window, text="Update Vacancy", font="Corbel 10", width=20, height=2,bd=5, command=update_vacancy).grid(row=7, sticky="w", padx=30, pady=20)
       ...
       Label(update_vac_window, text=" ", bg="#2C5F2D").grid(row=8)
...
def clear_cmd_update_vacancy2():
      ...
      # mid_var.set("")
       ...
       #rno_var.set("")
       update_vac_window = Toplevel()
       update_vac_window.geometry("400x540")
       update_vac_window.resizable(0, 0)
       update_vac_window.title("Create Reservation")
       update_vac_window.configure(bg="#2C5F2D")
       Label(update_vac_window, text="   ╔═══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C").grid(row=1)
       Label(update_vac_window, text=" Create Vacancy.",bg="#2C5F2D", fg="#F2AA4C", font="Corbel 20 bold").grid(row=2)
       Label(update_vac_window, text="   ╚═══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
       ...
       def update_vacancy():
              ...
              rno=rno_var.get()
              #cur.execute("Room no.")
              cur.execute("update room set Membership_ID='%s' where Room_no='%s'"%(mid,rno))
              db.commit()
              cur.execute("update room set First_Name='%s' where Room_no='%s'"%(fnm,rno))
              db.commit()
              cur.execute("update room set Last_Name='%s' where Room_no='%s'"%(lnm,rno))
              db.commit()
              cur.execute("update room set Booking_amount='%s' where Room_no='%s'"%(bkm,rno))
              db.commit()
              cur.execute("update room set Occupancy="R" where Room_no='%s'"%rno)
              db.commit()
       #ROOM NUMBER ENTRY
       Label(update_vac_window, text="Enter Room No: ", bg="#2C5F2D",font="Corbel 15 bold", fg="#F2AA4C").grid(row=5,sticky='w',pady=10)
       rno_entry = (Entry(update_vac_window, width=15, font="Consolas 15",textvariable=rno_var).grid(row=5, sticky="e",pady=10))
       #MEMBERSHIP ID ENTRY
       Label(update_vac_window, text="Enter MID: ", bg="#2C5F2D",font="Corbel 15 bold", fg="#F2AA4C").grid(row=6,sticky='w',pady=10)
       MID_entry = (Entry(update_vac_window, width=15, font="Consolas 15",textvariable=mid_var).grid(row=6, sticky="e",pady=10))
       #FIRST NAME ENTRY
       Label(update_vac_window, text="Enter First Name: ", bg="#2C5F2D",font="Corbel 15 bold", fg="#F2AA4C").grid(row=7,sticky='w',pady=10)
       fnm_entry = (Entry(update_vac_window, width=15, font="Consolas 15",textvariable=fnm_var).grid(row=7, sticky="e",pady=10))
       #LAST NAME ENTRY
       Label(update_vac_window, text="Enter Last Name: ", bg="#2C5F2D",font="Corbel 15 bold", fg="#F2AA4C").grid(row=8,sticky='w',pady=10)
       lnm_entry = (Entry(update_vac_window, width=15, font="Consolas 15",textvariable=lnm_var).grid(row=8, sticky="e",pady=10))
       #BOOKING AMOUNT ENTRY
       Label(update_vac_window, text="Booking Amount: ", bg="#2C5F2D",font="Corbel 15 bold", fg="#F2AA4C").grid(row=9,sticky='w',pady=10)
       bkm_entry = (Entry(update_vac_window, width=15, font="Consolas 15",textvariable=bkm_var).grid(row=9, sticky="e",pady=10))       
       ...
       def clear_input():
           ...
           mid_var.set("")
       #BUTTONS
       Label(update_vac_window, text=" ", bg="#2C5F2D").grid(row=11)
       Button(update_vac_window, text="Update Vacancy", font="Corbel 10", width=20, height=2,bd=5, command=update_vacancy).grid(row=12, sticky="w", padx=25)
       Button(update_vac_window, text="Clear", font="Corbel 10", width=20,height=2, bd=5, command=clear_input).grid(row=12, sticky="e")
       Label(update_vac_window, text=" ", bg="#2C5F2D").grid(row=13)
...
def update_rooms_options():
      update_rooms_window = Toplevel()
      update_rooms_window.geometry("400x350")
      update_rooms_window.resizable(0, 0)
      update_rooms_window.title("Rooms Details Update Options")
      update_rooms_window.configure(bg="#2C5F2D")
      Label(update_rooms_window, text=" ╔══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C", padx=20).grid(row=1)
      Label(update_rooms_window, text=" Update Rooms", bg="#2C5F2D",fg="#F2AA4C", font="Corbel 20 bold", padx=90).grid(row=2)
      Label(update_rooms_window, text=" ╚══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
      Label(update_rooms_window, text="", bg="#2C5F2D").grid(row=4)
      Button(update_rooms_window, text="Update Vacancy", width=20,height=2, command=clear_cmd_update_vacancy, bd=5).grid(row=5)
      Label(update_rooms_window, text="", bg="#2C5F2D").grid(row=6)
      Button(update_rooms_window, text="Update Reservation", width=20,height=2, command=clear_cmd_update_vacancy2, bd=5).grid(row=7)
      Label(update_rooms_window, text="", bg="#2C5F2D").grid(row=8)
      Button(update_rooms_window, text="Close Window", width=20,height=2, command=update_rooms_window.destroy, bd=5).grid(row=9)
...
def display_rooms_options():
      ...
      display_rooms_window.configure(bg="#2C5F2D")
      Label(display_rooms_window, text=" ╔══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C", padx=20).grid(row=1)
      Label(display_rooms_window, text="Search Rooms", bg="#2C5F2D",fg="#F2AA4C", font="Corbel 20 bold", padx=90).grid(row=2)
      Label(display_rooms_window, text=" ╚══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3)
      Label(display_rooms_window, text="", bg="#2C5F2D").grid(row=4)
      Button(display_rooms_window, text="Show All Rooms", width=20,height=2, command=display_all_rooms, bd=5).grid(row=5)
      Label(display_rooms_window, text="", bg="#2C5F2D").grid(row=6)
      Button(display_rooms_window, text="Search By Room Number", width=20,height=2, command=clear_cmd_rooms, bd=5).grid(row=7)
      Label(display_rooms_window, text="", bg="#2C5F2D").grid(row=8)
      Button(display_rooms_window, text="Search By Occupancy", width=20,height=2, command=clear_cmd_vacancy, bd=5).grid(row=9)
      Label(display_rooms_window, text="", bg="#2C5F2D").grid(row=10)
...
def rooms_window():
    ...
    rooms_page.geometry("945x500")
    rooms_page.resizable(0, 0)
    cur.execute("SELECT * FROM room")
    myresult = cur.fetchall()
    main_frame = Frame(rooms_page, bg="#2C5F2D")
    main_frame.pack(fill=BOTH, expand=1)
    my_canvas = Canvas(main_frame, bg="#2C5F2D")
    my_canvas.pack(side=LEFT, fill=BOTH, expand=1)
    my_scrollbar = ttk.Scrollbar(main_frame, orient=VERTICAL, command=my_canvas.yview)
    ...
    second_frame = Frame(my_canvas, bg="#2C5F2D")
    my_canvas.create_window((0, 0), window=second_frame, anchor="nw")
    Label(second_frame, text=" ╔══════════╗", bg='#2C5F2D',font="Corbel 20 bold", fg="#F2AA4C", padx=25).grid(row=1,column=1)
    Label(second_frame, text="Rooms", bg="#2C5F2D", fg="#F2AA4C",font="Corbel 20 bold", padx=100).grid(row=2,column=1)
    Label(second_frame, text=" ╚══════════╝", bg="#2C5F2D",font="Corbel 20 bold", fg="#F2AA4C").grid(row=3,column=1)
    Label(second_frame, text="", bg="#2C5F2D").grid(row=4)
    details = Label(second_frame, text=tabulate(myresult, headers=['Membership ID', 'First Name', 'Last Name', 'Booking Amount', 'Room Number','Occupancy'], tablefmt='fancy_grid'), bg="#2C5F2D", fg="#F2AA4C", font="Consolas 12 ").grid(pady=10, padx=20, row=5, column=1)
    Label(second_frame, text="", bg="#2C5F2D").grid(row=6)
    button_frame = Frame(my_canvas, bg="#2C5F2D")
    my_canvas.create_window((0, 0), window=button_frame, anchor="sw")
    button_frame.pack(side=BOTTOM)
    Button(button_frame, text="Search Rooms", width=20, height=2,command=display_rooms_options, bd=5).grid(row=0, column=0, padx=30, pady=15)
    Button(button_frame, text="Update Room", width=20, height=2,command=update_rooms_options, bd=5).grid(row=0, column=1, padx=30)
    Button(button_frame, text="Close Window", font="Corbel 10", width=20, height=2,bd=5, command=rooms_page.destroy).grid(row=0, column=4, padx=30)


#___________________________________
#EMPLOYEE WINDOW AND FUNCTIONS
